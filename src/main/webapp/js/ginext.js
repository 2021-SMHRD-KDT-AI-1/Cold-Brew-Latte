...
let questions = [
    ...
    new Question('사용자의 명령어를 인식하여 프로그램을 호출하고 명령을 수행하는 명령어 해석기를 무엇이라고 하는가?', ['NUR(Not Used Recently)','커널(Kernel)', '쉘(Shell)', '비선점(Non-preemptive) 스케줄링'], '커널(Kernel)'),
...
function updateQuiz(){
    ...
    $(".quiz1").html(
    '<p class="question">'+questionIndex+"."+quiz.questions[quiz.questionNum].text+'</p>'+
    '<div class="answers">'+
    ...
    $(".quiz2").html('<h2>&lt; 문제<span class="num">'+questionIndex+'</span>&gt;</h2>'+
    ...
    $(".quiz3").html('<h2>&lt; 문제<span class="num">'+questionIndex+'</span>&gt;</h2>'+
    '<p class="question">'+quiz.questions[quiz.questionNum].text+'</p>'+
    '<div class="answers">'+
    '<label><input type="radio" name="question" class="select" value='+quiz.questions[quiz.questionNum].choice[0]+' ><span >'+quiz.questions[quiz.questionNum].choice[0]+'</span></label>' +
    '<label><input type="radio" name="question" class="select" value='+quiz.questions[quiz.questionNum].choice[1]+' ><span >'+quiz.questions[quiz.questionNum].choice[1]+'</span></label>' +
    ...
    $(".quiz4").html('<h2>&lt; 문제<span class="num">'+questionIndex+'</span>&gt;</h2>'+
    '<p class="question">'+quiz.questions[quiz.questionNum].text+'</p>'+
    '<div class="answers">'+
    '<label><input type="radio" name="question" class="select" value='+quiz.questions[quiz.questionNum].choice[0]+' ><span >'+quiz.questions[quiz.questionNum].choice[0]+'</span></label>' +
    '<label><input type="radio" name="question" class="select" value='+quiz.questions[quiz.questionNum].choice[1]+' ><span >'+quiz.questions[quiz.questionNum].choice[1]+'</span></label>' +
    ...
    $(".quiz5").html('<h2>&lt; 문제<span class="num">'+questionIndex+'</span>&gt;</h2>'+
    '<p class="question">'+quiz.questions[quiz.questionNum].text+'</p>'+
    '<div class="answers">'+
    '<label><input type="radio" name="question" class="select" value='+quiz.questions[quiz.questionNum].choice[0]+' ><span >'+quiz.questions[quiz.questionNum].choice[0]+'</span></label>' +
    '<label><input type="radio" name="question" class="select" value='+quiz.questions[quiz.questionNum].choice[1]+' ><span >'+quiz.questions[quiz.questionNum].choice[1]+'</span></label>' +
